...
void DAC7578::setVoltage(uint8_t channel, float v) {
    uint16_t data = v * (2 << 12) / 3.3;
    this->writeToRegister(DAC7578_WRITE_AND_UPDATE_ALL_COMMAND, channel, data);
}
void DAC7578::setVoltageAll(float v) {
    uint16_t data = v * (2 << 12) / 3.3;
    this->writeToRegister(DAC7578_WRITE_AND_UPDATE_ALL_COMMAND, DAC7578_CHANNEL_ALL, data);
}
...
void DAC7578::clearAllZeroScale(uint8_t index) {
    ...
    this->writeToRegister(DAC7578_WRITE_TO_CLEAR_COMMAND, 0, data);
}
...
void DAC7578::softwareReset(uint8_t control, uint8_t index) {
    ...
    writeToRegister(DAC7578_SOFTWARE_RESET_COMMAND, 0, data);
}

// LDAC
void DAC7578::LDACSetAsync(uint8_t channels, uint8_t index) {
    uint16_t data = (channels << 15);
    writeToRegister(DAC7578_WRITE_TO_LDAC_COMMAND, 0, data);
}
...
void DAC7578::powerOn(uint8_t channels, uint8_t index) {
    ...
    writeToRegister(DAC7578_POWER_DOWN_DAC_COMMAND, 0, data);
}
